...
local function spawnBomb(drone, info, opts)
	...
	bomb.Size = Vector3.new(1.8, 0.55, 0.55)
	bomb.Color = Color3.fromRGB(70, 70, 60)
	bomb.Material = Enum.Material.Metal
	bomb.CanCollide = false
	bomb.CFrame = primary.CFrame * CFrame.new(0, -1.4, -1.2) * CFrame.Angles(0, math.rad(90), 0)
...
local function spawnDroneMissile(drone, info, dir, opts)
	...
	missile.Size = Vector3.new(3.2, 0.42, 0.42)
	missile.Color = Color3.fromRGB(200, 190, 140)
	missile.Material = Enum.Material.Metal
	missile.CanCollide = false
	missile.CFrame = CFrame.lookAt(primary.Position + dir * 6, primary.Position + dir * 20) * CFrame.Angles(0, math.rad(90), 0)
...
local function spawnSwarmUnits(player, leaderModel, cfg)
	local units = {}
	local origin = PAD + Vector3.new(0, 18, 0)
	for i = 1, cfg.Ammo do
		local ang = (i / cfg.Ammo) * math.pi * 2
		local pos = origin + Vector3.new(math.cos(ang) * 16, (i % 3) * 2.5, math.sin(ang) * 16)
		...
		align.MaxTorque = 1e7
		align.CFrame = CFrame.Angles(0, math.pi, 0)
		...
		lvel.VectorVelocity = Vector3.new(0, 22, 28)
		pcall(function()
			lvel.ForceLimitMode = Enum.ForceLimitMode.Magnitude
			lvel.VelocityConstraintMode = Enum.VelocityConstraintMode.Vector
		end)
		lvel.Parent = primary
		for _, part in ipairs(unit:GetDescendants()) do
			if part:IsA("BasePart") then
				...
				part.AssemblyLinearVelocity = Vector3.new(0, 22, 28)
			end
		end
...
function DroneService.Launch(player, droneId)
	...
	-- покупка ОДИН РАЗ навсегда: дальше запуски бесплатные
	if cfg.Price > 0 and not Economy.IsUnlocked(player, droneId) then
		if not Economy.TrySpend(player, cfg.Price) then return nil, "Не хватает монет" end
		Economy.Unlock(player, droneId)
		fireRemote(Config.REM.OperFX, player, { type = "msg", text = cfg.Name .. " куплен НАВСЕГДА! Следующие запуски бесплатны." })
	end
	...
	-- PivotTo пока детали Anchored: иначе сварка рвёт модель, и двигается только корпус.
	local launchPos = PAD + Vector3.new(0, 16, 0)
	local launchCF = CFrame.lookAt(launchPos, launchPos + Vector3.new(0, 4, 40))
	model:PivotTo(launchCF)
	model.Parent = workspace.Drones
	local primary = model.PrimaryPart
	if not primary then return nil, "Нет корпуса" end
	...
	align.MaxTorque = 1e7
	align.CFrame = launchCF - launchCF.Position
	align.Parent = primary
	local lvel = Instance.new("LinearVelocity")
	lvel.Name = "FlightVel"
	lvel.Attachment0 = att
	lvel.MaxForce = 1e7
	lvel.RelativeTo = Enum.ActuatorRelativeTo.World
	lvel.VectorVelocity = Vector3.new(0, 36, 62)
	pcall(function()
		lvel.ForceLimitMode = Enum.ForceLimitMode.Magnitude
		lvel.VelocityConstraintMode = Enum.VelocityConstraintMode.Vector
	end)
	...
	for _, part in ipairs(model:GetDescendants()) do
		if part:IsA("BasePart") then
			part.Anchored = false
			part.CanCollide = (part == primary)
			part.AssemblyLinearVelocity = Vector3.new(0, 36, 62)
		end
	end
...
local function bindRemotes()
	...
	if returnEv then
		returnEv.OnServerEvent:Connect(function(player)
			...
			if dist < 90 then
				-- посадка: дрон возвращается в ангар (запуск уже бесплатный — без рефанда)
