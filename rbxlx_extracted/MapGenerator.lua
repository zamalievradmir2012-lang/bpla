...
local function addTrafficLight(x, z, rotY, axis)
	...
	for _, c in ipairs(colors) do
		local b = P({ Size = Vector3.new(0.55, 0.55, 0.2), CFrame = box.CFrame * CFrame.new(c[2] + Vector3.new(0, 0, -0.35)), Color = c[1], Material = MAT.Plastic, Transparency = 0.4 })
...
local function residential(cx, cz, floors, opts)
	...
	for f = 1, floors do
		...
		-- ===== ИНТЕРЬЕР: комнаты, мебель, свет =====
		local fy = (f == 1) and 1.1 or (y0 - 0.4) -- уровень пола этажа
		local rnd = Random.new(math.floor(math.abs(cx * 7 + cz * 13 + f * 131)) % 999983 + 1)
		local wallC = Color3.fromRGB(216, 208, 194)
		local ip = function(props, hp) return reg(P(props), hp, "Prop", bld, f) end
		-- лифтовая шахта / лестничный узел
		local core = ip({ Size = Vector3.new(6, 8, 5), CFrame = CFrame.new(cx + w / 4, fy + 4, cz), Color = Color3.fromRGB(150, 150, 148), Material = MAT.Concrete }, 220)
		if f == 1 then
		...
		-- перегородка вдоль дома с дверным проёмом
		local segW = (w - 8) / 2 - 2
		for _, s in ipairs({ -1, 1 }) do
			ip({ Size = Vector3.new(segW, 8, 0.6), CFrame = CFrame.new(cx + s * (segW / 2 + 3), fy + 4, cz - d * 0.12), Color = wallC, Material = MAT.Concrete }, 40)
		end
		-- поперечная перегородка (не на каждом этаже — планировки разные)
		if f % 3 ~= 0 then
			ip({ Size = Vector3.new(0.6, 8, d * 0.45), CFrame = CFrame.new(cx - w * 0.18, fy + 4, cz + d * 0.18), Color = wallC, Material = MAT.Concrete }, 40)
		end
		-- мебель
		local function bed(x, z, rot)
			local cf = CFrame.new(x, fy + 0.7, z) * CFrame.Angles(0, rot, 0)
			ip({ Size = Vector3.new(5, 1.4, 7), CFrame = cf, Color = Color3.fromRGB(112, 84, 62), Material = MAT.Wood }, 20)
			ip({ Size = Vector3.new(4.4, 0.6, 2.6), CFrame = cf * CFrame.new(0, 1, -1.8), Color = Color3.fromRGB(230, 230, 234), Material = MAT.Plastic }, 10)
		end
		local function wardrobe(x, z, rot)
			ip({ Size = Vector3.new(4.5, 7, 1.6), CFrame = CFrame.new(x, fy + 3.5, z) * CFrame.Angles(0, rot, 0), Color = Color3.fromRGB(96, 66, 46), Material = MAT.Wood }, 25)
		end
		local function sofa(x, z, rot)
			ip({ Size = Vector3.new(6.5, 2.6, 2.6), CFrame = CFrame.new(x, fy + 1.3, z) * CFrame.Angles(0, rot, 0), Color = Color3.fromRGB(80 + rnd:NextInteger(0, 90), 95, 115), Material = MAT.Plastic }, 20)
		end
		local function tableSet(x, z, rot)
			local cf = CFrame.new(x, fy, z) * CFrame.Angles(0, rot, 0)
			ip({ Size = Vector3.new(4.5, 0.35, 3), CFrame = cf * CFrame.new(0, 2.4, 0), Color = Color3.fromRGB(130, 96, 62), Material = MAT.Wood }, 15)
			ip({ Size = Vector3.new(0.6, 2.4, 0.6), CFrame = cf * CFrame.new(0, 1.2, 0), Color = Color3.fromRGB(90, 70, 50), Material = MAT.Metal }, 10)
		end
		local fz1 = cz - d / 4 + rnd:NextNumber(-1.5, 1.5)
		local fz2 = cz + d / 4 + rnd:NextNumber(-1.5, 1.5)
		bed(cx - w / 4 + rnd:NextNumber(-2, 2), fz1, rnd:NextInteger(0, 1) * math.rad(90))
		wardrobe(cx + w / 2 - 1.5, fz1, math.rad(90))
		sofa(cx + w / 4 + rnd:NextNumber(-2, 2), fz2, rnd:NextInteger(0, 1) * math.rad(180))
		tableSet(cx - w / 4 + rnd:NextNumber(-2, 2), cz + d / 4 + 2, 0)
		-- потолочные светильники (светятся как настоящие)
		ip({ Size = Vector3.new(2.6, 0.25, 2.6), CFrame = CFrame.new(cx - w / 4, y0 + 8.6, cz + d / 5), Color = Color3.fromRGB(255, 240, 200), Material = MAT.Neon }, 5)
		ip({ Size = Vector3.new(2.6, 0.25, 2.6), CFrame = CFrame.new(cx + w / 4, y0 + 8.6, cz - d / 5), Color = Color3.fromRGB(255, 240, 200), Material = MAT.Neon }, 5)
	end
...
local function glassTower(cx, cz, floors, opts)
	...
	for f = 1, floors do
		...
		-- ===== ИНТЕРЬЕР ОФИСА =====
		local fy = (f == 1) and 1.1 or (y0 - 0.4)
		local core = reg(P({ Size = Vector3.new(8, 8, 8), CFrame = CFrame.new(cx, fy + 4, cz), Color = Color3.fromRGB(72, 74, 80), Material = MAT.Concrete }), 260, "Prop", bld, f)
		if f == 1 then
			local lamp = Instance.new("PointLight")
			lamp.Color = Color3.fromRGB(220, 235, 255)
			lamp.Brightness = 1.1
			lamp.Range = 45
			lamp.Shadows = false
			lamp.Parent = core
		...
		-- офисные столы
		for _, s in ipairs({ -1, 1 }) do
			reg(P({ Size = Vector3.new(11, 2.4, 3), CFrame = CFrame.new(cx + s * 9, fy + 1.2, cz + s * 4), Color = Color3.fromRGB(150, 120, 85), Material = MAT.Wood }), 25, "Prop", bld, f)
		end
		-- потолочная световая линия
		reg(P({ Size = Vector3.new(9, 0.25, 1.4), CFrame = CFrame.new(cx, y0 + 8.6, cz + 8), Color = Color3.fromRGB(230, 240, 255), Material = MAT.Neon }), 5, "Prop", bld, f)
	end
...
local function warehouse(cx, cz, alongX)
	...
	-- ===== ИНТЕРЬЕР: ящики и паллеты =====
	for i = 1, 5 do
		local bx = -w / 2 + 10 + ((i - 1) % 3) * 14
		local bz = -d / 2 + 9 + math.floor((i - 1) / 3) * 12
		reg(P({ Size = Vector3.new(5, 5, 5), CFrame = cf * CFrame.new(bx, 3.5, bz), Color = Color3.fromRGB(140, 100, 60), Material = MAT.Wood }), 35, "Prop", bld, 1)
	end
	return bld
end
...
local function school(cx, cz)
	...
	-- ===== ИНТЕРЬЕР: классы с партами =====
	for row = 1, 3 do
		for col = 1, 4 do
			reg(P({ Size = Vector3.new(5, 2.4, 3), CFrame = CFrame.new(cx - 27 + col * 14, 2.3, cz - 9 + row * 9), Color = Color3.fromRGB(150, 110, 70), Material = MAT.Wood }), 20, "Prop", bld, 1)
		end
	end
	reg(P({ Size = Vector3.new(20, 5, 0.5), CFrame = CFrame.new(cx, 6, cz - d / 2 + 1), Color = Color3.fromRGB(30, 60, 40), Material = MAT.Slate }), 15, "Prop", bld, 1)
...
local function mall(cx, cz)
	...
	-- ===== ИНТЕРЬЕР: торговые киоски =====
	for i = 1, 6 do
		local kx = cx - 40 + (i - 1) * 16
		local kz = cz + ((i % 2 == 0) and 12 or -12)
		reg(P({ Size = Vector3.new(9, 7, 6), CFrame = CFrame.new(kx, 4.6, kz), Color = Color3.fromRGB(185 + (i * 13) % 50, 180 + (i * 7) % 30, 170), Material = MAT.Plastic }), 60, "Prop", bld, 1)
	end
	reg(P({ Size = Vector3.new(60, 0.3, 3), CFrame = CFrame.new(cx, 20, cz), Color = Color3.fromRGB(255, 240, 210), Material = MAT.Neon }), 10, "Prop", bld, 1)
	-- неоновые вывески
	local signs = {
