...
local function buildBunker()
	...
	for _, sx in ipairs({ -1, 1 }) do
		...
		Util.part({ Size = Vector3.new(W, H, 2), CFrame = CFrame.new(base + Vector3.new(0, H / 2, sx * D / 2)), Color = Color3.fromRGB(52, 56, 60), Material = Enum.Material.Concrete }, folder)
	end
	-- антенны на крыше
	for i = 1, 3 do
	...
	-- взлётная площадка на крыше (PAD_POS = 46)
	Util.part({ Size = Vector3.new(50, 1, 50), CFrame = CFrame.new(Config.PAD_POS - Vector3.new(0, 0.5, 0)), Color = Color3.fromRGB(30, 32, 36), Material = Enum.Material.Concrete }, folder)
	Util.part({ Shape = Enum.PartType.Cylinder, Size = Vector3.new(0.3, 34, 34), CFrame = CFrame.new(Config.PAD_POS + Vector3.new(0, 0.2, 0)) * CFrame.Angles(0, 0, math.rad(90)), Color = Color3.fromRGB(80, 220, 120), Material = Enum.Material.Neon, CanCollide = false }, folder)
	-- посадочный маяк
	...
	light.Range = 60
	local lampPart = Util.part({ Size = Vector3.new(1, 1, 1), Transparency = 1, CFrame = CFrame.new(base + Vector3.new(0, H - 4, 0)) }, folder)
	...
	for i = 1, 4 do
		...
		seat.Size = Vector3.new(4, 0.8, 4)
		seat.Anchored = true
		seat.CFrame = CFrame.new(base + Vector3.new(-15 + i * 10, 1.5, 18))
		seat.Color = Color3.fromRGB(30, 34, 40)
		seat.Parent = folder
	end
end
...
local function onCharacterAdded(player, char)
	...
	if player.Team == teamOps then
		-- в бункер
		local seatPos = Config.BUNKER_POS + Vector3.new(math.random(-15, 15), 3, 14)
		char:PivotTo(CFrame.new(seatPos))
	elseif player.Team == teamDef then
		local spawn = Config.DEF_SPAWNS[math.random(1, #Config.DEF_SPAWNS)]
		char:PivotTo(CFrame.new(spawn + Vector3.new(0, 3, 0)))
	end
	-- иначе — Neutral, спавн в лобби через SpawnLocation
end
...
local function playRound()
	...
	DroneService.SetRayExcludes({ workspace.Debris, workspace.FX, workspace.Drones })
	Economy.ResetRound()
end
...
local function main()
	...
	Players.CharacterAutoLoads = false -- включим после постройки лобби
	setupLighting()
	...
	-- спавн игроков
	Players.PlayerAdded:Connect(function(pl)
		pl.CharacterAdded:Connect(function(char) onCharacterAdded(pl, char) end)
	end)
	for _, pl in ipairs(Players:GetPlayers()) do
		pl.CharacterAdded:Connect(function(char) onCharacterAdded(pl, char) end)
		if pl.Character then task.spawn(onCharacterAdded, pl, pl.Character) end
	end
